...
fn test_xor_convergence_dynamics() {
    ...
    // Model: 2 -> 4 -> 1
    let x_in = gb.val(array![[0.0, 0.0]]); // Placeholder
    
    // Hidden Layer
    let w1 = gb.param(Array2::from_elem((2, 4), 0.5));
    let b1 = gb.param(Array2::zeros((1, 4)));
    let h1 = gb.linear(x_in, w1, b1);
    let a1 = gb.sigmoid(h1);
    
    // Output Layer
    let w2 = gb.param(Array2::from_elem((4, 1), 0.5));
    let b2 = gb.param(Array2::zeros((1, 1)));
    ...
    graph.backward(out, array![[1.0]]).unwrap();
