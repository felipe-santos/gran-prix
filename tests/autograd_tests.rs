...
fn test_autograd_simple_chain() {
    ...
    let w = gb.param(array![[0.5, 0.5]].into_dyn().into());
    let sum = gb.add(x, w);
    let out = gb.relu(sum);
    
    let result = graph.execute(out).unwrap();
    assert_eq!(result, array![[1.5, 0.0]].into_dyn().into());
    ...
    assert_eq!(*grad_w, array![[1.0, 0.0]].into_dyn().into());
}
...
fn test_autograd_matmul() {
    ...
    // y = x * w
    let x = gb.val(array![[1.0, 2.0]].into_dyn().into());
    let w = gb.param(array![[0.5, 0.1], [0.2, 0.4]].into_dyn().into());
    ...
    // [[1], [2]] * [[1, 1]] = [[1, 1], [2, 2]]
    let grad_w = graph.get_gradient(w).unwrap();
    assert_eq!(*grad_w, array![[1.0, 1.0], [2.0, 2.0]].into_dyn().into());
}
