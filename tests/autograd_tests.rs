...
fn test_autograd_matmul() {
    ...
    // y = x * w
    let x = gb.val(array![[1.0, 2.0]]);
    let w = gb.param(array![[0.5, 0.1], [0.2, 0.4]]);
    ...
    // [[1], [2]] * [[1, 1]] = [[1, 1], [2, 2]]
    let grad_w = graph.get_gradient(w).unwrap();
    assert_eq!(*grad_w, array![[1.0, 1.0], [2.0, 2.0]]);
}
