...
fn test_multilayer_gradient_flow() {
    ...
    let x_data = Tensor::from_elem(&[1, 2], 1.0);
    let x = gb.val(x_data);

    // Layer 1
    let w1 = gb.param(Tensor::from_elem(&[2, 8], 0.1));
    let b1 = gb.param(Tensor::from_elem(&[1, 8], 0.0));
    let l1 = gb.matmul(x, w1);
    let l1 = gb.add(l1, b1);
    let a1 = gb.tanh(l1);

    // Layer 2
    let w2 = gb.param(Tensor::from_elem(&[8, 4], 0.1));
    let b2 = gb.param(Tensor::from_elem(&[1, 4], 0.0));
    let l2 = gb.matmul(a1, w2);
    let l2 = gb.add(l2, b2);
    let a2 = gb.tanh(l2);

    // Layer 3 (Output)
    let w3 = gb.param(Tensor::from_elem(&[4, 1], 0.1));
    let b3 = gb.param(Tensor::from_elem(&[1, 1], 0.0));
    ...
    let sum_abs_w3: f32 = g_w3.as_slice().unwrap().iter().map(|x| x.abs()).sum();
    let sum_abs_w2: f32 = g_w2.as_slice().unwrap().iter().map(|x| x.abs()).sum();
    let sum_abs_w1: f32 = g_w1.as_slice().unwrap().iter().map(|x| x.abs()).sum();
    ...
    let val1 = graph.execute(out).unwrap().as_slice().unwrap()[0];
    println!("Initial Output: {}", val1);

    // Mutate W1 via the param store
    {
        let node = &graph.nodes()[w1.0];
        let param_id = node.param_id().expect("W1 should be a Param node");
        graph.params_mut().tensor_mut(param_id).map_inplace(|v| *v += 1.0).unwrap();
    }
    graph.sync_params().unwrap();

    let val2 = graph.execute(out).unwrap().as_slice().unwrap()[0];
