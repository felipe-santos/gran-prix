...
fn test_cpu_matmul() {
    ...
    let expected = array![[19.0, 22.0], [43.0, 50.0]].into_dyn().into();
...
fn test_cpu_sigmoid() {
    ...
    assert!((res.view()[[0, 0]] - 0.5).abs() < 1e-6);
    assert!((res.view()[[0, 1]] - 0.7310586).abs() < 1e-6);
}
...
fn test_cpu_relu() {
    ...
    let expected = array![[0.0, 2.0, 0.0]].into_dyn().into();
    assert_eq!(res, expected);
}
...
fn test_cpu_add_relu_fused() {
    ...
    let expected = array![[0.0, 2.0]].into_dyn().into();
    assert_eq!(res, expected);
}
