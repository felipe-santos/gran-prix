...
impl CUDABackend {
    pub fn new(device_index: usize) -> Result<Self> {
        let device = CudaDevice::new(device_index)
            .map_err(|e| anyhow!("Failed to initialize CUDA device {}: {:?}", device_index, e))?;
    ...
    fn get_cuda_slice<'a>(&self, t: &'a Tensor) -> Result<Arc<cudarc::driver::CudaSlice<f32>>> {
        match t.storage() {
...
impl Backend for CUDABackend {
    fn relu(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let mut out_slice = self.device.alloc_zeros::<f32>(n)
            .map_err(|e| anyhow!("CUDA alloc failed: {:?}", e))?;
    ...
    fn matmul_t(&self, a: &Tensor, b: &Tensor, trans_a: bool, trans_b: bool) -> Result<Tensor> {
        ...
        if k != k_b {
            return Err(anyhow!("MatMul k-dimension mismatch: {} vs {}", k, k_b));
        }
