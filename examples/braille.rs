...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let inputs_data: Tensor = Tensor::from_shape_vec(&[5, 6], vec![
    ...
    let targets_data: Tensor = Tensor::from_shape_vec(&[5, 1], vec![0.0, 29.0, 2.0, 32.0, 48.0])?;
    ...
    for i in 0..5 {
        println!("Pattern {:?} | Expected: {:.2} | Predicted: {:.4}",
            i, targets_data.get_2d(i, 0).unwrap(), final_output.get_2d(i, 0).unwrap());
    }
