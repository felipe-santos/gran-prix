...
fn main() -> anyhow::Result<()> {
    println!("Gran-Prix Static Safety Demo: Shape Validation");
    ...
    let x = gb.val(Tensor::from_shape_vec(&[1, 2], vec![1.0, 2.0])?); // [1, 2]
    let w = gb.val(Tensor::from_shape_vec(&[2, 2], vec![0.5, 0.1, 0.2, 0.4])?); // [2, 2]
    let _b = gb.val(Tensor::from_shape_vec(&[1, 2], vec![0.1, 0.1])?);
    ...
    let x2 = bgb.val(Tensor::from_shape_vec(&[1, 3], vec![1.0, 2.0, 3.0])?); // [1, 3]
    let w2 = bgb.val(Tensor::from_shape_vec(&[2, 2], vec![0.5, 0.1, 0.2, 0.4])?); // [2, 2]
